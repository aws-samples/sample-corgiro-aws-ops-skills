...
- Redis OSS versions AWS documents as deprecated patch releases are 🔴 Critical regardless of their major version's support date, since Extended Support covers only the latest patch version of each major version.
...
Flag two facts that change the operator's decision:
...
4. Per-account breakdown - replication group, engine, version, node type, node count, `AutoMinorVersionUpgrade`, cluster mode, Multi-AZ
...
- **Auto minor version upgrade is supported** (unlike EKS and OpenSearch). `AutoMinorVersionUpgrade: false` on a cache near EOL is a finding worth surfacing - it means minor security patches are not being picked up automatically.
